...
Set stream = fs.CreateTextFile(pPathFile, True, False)
...
' Write header line
stream.WriteLine (headerContent)
...
While (ActiveCell.Offset(rowOff, 0).Value <> "END-END-END-END")
    ...
    stream.WriteLine (dataContent) 'Write each line data
    
    ' Next row
    rowOff = rowOff + 1
	
	' If offset = 30K then active cell at offset = 30K and reset offset = 0
    If rowOff = 30000 Then
        ActiveCell.Offset(rowOff, 0).Select
        rowOff = 0
    End If
